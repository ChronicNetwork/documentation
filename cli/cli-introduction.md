---
description: >-
  A general introduction Chronic cli along with a brief description of commands and
  flags
coverY: 0
---
...
`chtd` is a command line client for the Chronic network. Chronic users can use `chtd` to send transactions to the Chronic network and query the blockchain data.

{% hint style="info" %}
See [here](../validators/getting-setup.md) for instructions on installing `chtd`.
{% endhint %}

### Working Directory <a href="#working-directory" id="working-directory"></a>

The default working directory for the `chtd` is `$HOME/.cht`, which is mainly used to store configuration files and blockchain data. The Chronic `key` data is saved in the working directory of `chtd`. You can also specify the `chtd` working directory by using the `--home` flag when executing `chtd`.&#x20;

### Connecting to a Full-Node

By default, `chtd` uses `tcp://localhost:26657` as the RPC address to connect to the Chronic network. This default configuration assumes that the machine executing `chtd` is running as a full-node.

The RPC address can be specified to connect to any full-node with an exposed RPC port by adding the `--node` flag when executing `chtd`
...
| --home          | string |          | $HOME/.cht    | Directory for config and data        |
